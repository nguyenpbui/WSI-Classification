...
if train:
    ...
    dataloader_train = torch.utils.data.DataLoader(dataset=dataset_train, batch_size=batch_size, num_workers=10, 
...
dataloader_val = torch.utils.data.DataLoader(dataset=dataset_val, batch_size=batch_size, num_workers=10, 
...
optimizer = torch.optim.AdamW(model.parameters(), lr = learning_rate, weight_decay = 5e-4)       # best:5e-4, 4e-3
scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[30,60,90], gamma=0.3) # gamma=0.3  # 30,90,130 # 20,90,130 -> 150
# scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, milestones=[20,100], gamma=0.1) # gamma=0.3  # 30,90,130 # 20,90,130 -> 150

##################################
# weight = torch.tensor([0.184, 0.230, 0.413, 0.087, 0.087]).cuda()
