        ...
        while True:
            ...
            if True:
                try:
                    ...
                    gray = tile.convert('L')
                    bw = gray.point(lambda x: 0 if x<220 and x>50 else 1, 'F')
                    arr = np.array(np.asarray(bw))
                    avgBkg = np.average(bw)
                    bw = gray.point(lambda x: 0 if x<220 and x>50  else 1, '1')
...
class DeepZoomImageTiler(object):
    ...
    def __init__(self, dz, basename, format, associated, queue, slide, basenameJPG, xmlfile,
                  mask_type, xmlLabel, ROIpc, ImgExtension, SaveMasks, Mag, normalize, Fieldxml):
        self._dz = dz
    ...
    def _write_tiles(self):
        ...
        try:
            ...
            if ("png" in self._ImgExtension) | ("dcm" in self._ImgExtension) | ("tif" in self._ImgExtension):
                #Objective = self._ROIpc
                Objective = 1.
                Magnification = Objective
                print("input is png - will be tiled as such with %f" % Objective)
            else:
                return
        ...
        if True:
            ...
            # print("xml:")
            # print(xmldir)
            if (self._xmlfile != '') & (self._ImgExtension != 'png') & (self._ImgExtension != 'dcm'):
                # print("read xml file...")
...
class DeepZoomStaticTiler(object):
    ...
    def __init__(self, slidepath, basename, format, tile_size, overlap,
                limit_bounds, quality, workers, with_viewer, Bkg, basenameJPG, xmlfile, 
                mask_type, ROIpc, oLabel, ImgExtension, SaveMasks, Mag, normalize, Fieldxml):
        if with_viewer:
...
if __name__ == '__main__':
    ...
    for imgNb in range(len(files)):
        ...
        if ("dcm" in ImgExtension) :
            ...
            filename = os.path.join(opts.tmp_dcm, opts.basenameJPG + ".png")
            # print(filename)
        ...
        #	DeepZoomStaticTiler(filename, output, opts.format, opts.tile_size, opts.overlap, opts.limit_bounds, opts.quality, opts.workers, opts.with_viewer, opts.Bkg, opts.basenameJPG, opts.xmlfile, opts.mask_type, opts.ROIpc, '', ImgExtension, opts.SaveMasks, opts.Mag, opts.normalize, opts.Fieldxml).run()
            ...
            if os.path.isfile(xmldir):
                ...
                if (ImgExtension == ".png") | (ImgExtension == ".dcm") :
            ...
            try:
                ...
                DeepZoomStaticTiler(filename, output, opts.format, opts.tile_size, opts.overlap, opts.limit_bounds, 
                                    opts.quality, opts.workers, opts.with_viewer, opts.Bkg, opts.basenameJPG, opts.xmlfile, 
                                    opts.mask_type, opts.ROIpc, '', ImgExtension, opts.SaveMasks, opts.Mag, opts.normalize, opts.Fieldxml).run()
            except Exception as e:
                print("Failed to process file %s, error: %s" % (filename, sys.exc_info()[0]))
                print(e)
